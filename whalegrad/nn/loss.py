...
from whalegrad.engine.functions import Action, log
from whalegrad.engine.functions import sum as _sum
...
class BinaryCrossEntropy(Loss):
    ...
    def forward(self, outputs, targets, epsilon=1e-9):
        ...
        num_examples = self.get_num_examples(outputs.shape)
        entropy = _sum(
            (outputs * log(targets + epsilon))
            + ((1 - outputs) * (log(1 - targets + epsilon)))
        )
...
class SoftmaxCE(Action, Loss):
    ...
    def backward(self, outputs, targets):
        ...
        def sce_backward(ug):
            num_examples = self.get_num_examples(outputs.shape)
            probs = softmax.calc_softmax(outputs.data, axis=self.axis)
            return (ug / num_examples) * (
                probs - targets.data
            )  # CrossEntropy derivative w.r.t outputs

        outputs.set_grad_fn(sce_backward)
        assert (
            targets.requires_grad is False
        ), "Targets Tensor should have requires_grad=False"
