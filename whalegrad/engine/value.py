...
class Tensor:
  '''NumPy array wrapper with autograd support.
    ...
    Args:
        data (int/float/list/np.ndarray): Initial data.
        requires_grad (bool): Gradient calculation flag (default: False).
        requires_broadcasting (bool): Broadcasting flag (default: True).
        grad (np.ndarray): Gradient value (default: 0 if requires_grad else None).
        grad_fn: Operation.backward function (default: None).

    Raises:
        TypeError: If data type is not (int, float, list, np.ndarray).
    '''
